...
public class TextView extends View implements ViewTreeObserver.OnPreDrawListener {
    static final String TAG = "JotaTextView";
...
//                        Log.e( TAG , "===========================push> " + lastChange.start + ":" + lastChange.oldtext + ":" + lastChange.newtext );
     ...
     * back the way you want it.
     */
    public final void setJotaMovementMethod(MovementMethod movement) {
        mMovement = movement;
...
// Jota Text Editor
     ...
     * {@link LinkJotaMovementMethod} if {@link #setAutoLinkMask} has been
...
//                 * movement method, because setJotaMovementMethod() may call
//                 * setText() again to try to upgrade the buffer type.
//                 */
//                mText = text;
//
//                if (mLinksClickable) {
//                    setJotaMovementMethod(LinkJotaMovementMethod.getInstance());
//                }
//            }
//        }
        ...
        if (mSelectionModifierCursorController != null) {
            ...
            if (lastTapPosition >= 0) {
                ...
                if (lastTapPosition > mText.length()) {
                    Log.e(TAG, "Invalid tap focus position (" + lastTapPosition + " vs "
                            + mText.length() + ")");
...
// Jota Text Editor
    ...
    private void startTextSelectionMode() {
        if (!mIsInTextSelectionMode) {
            if (!hasSelectionController()) {
                Log.w(TAG, "TextView has no selection controller. Action mode cancelled.");
                return;
            }
...
// Jota Text Editor
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (mFastScroller != null) {
            mFastScroller.onScroll(this, t, getVisibleHeight() , mLayout.getHeight() );
        }
...
// Jota Text Editor
    public boolean movePage(boolean up , boolean extendSelection )
    {
        ...
        int lineHeight = mLayout.getLineTop(1) - mLayout.getLineTop(0);
        int visibleHeight = getVisibleHeight() ;
        int linesOfPage = visibleHeight / lineHeight;
        ...
        int offset = mLayout.getLineStart( nextLine );

        if ( extendSelection ){
            Selection.extendSelection((Spannable) mText, offset );
        }else{
            Selection.setSelection((Spannable) mText, offset,  offset );
        }
...
// Jota Text Editor
    public boolean centerCursor() {
        ...
        int maxpos = mLayout.getHeight();
        int selEnd = Selection.getSelectionEnd(mText);
        int line = mLayout.getLineForOffset(selEnd);
        int cursorline = mLayout.getLineTop(line);
        int visibleHeight = getVisibleHeight() ;
        int newpos =   cursorline -  visibleHeight*20/100 ;

        if ( newpos >= maxpos - visibleHeight ){
            newpos = maxpos - visibleHeight ;
        }
    ...
    private class InsertionPointCursorController implements CursorController {
        ...
        public void updatePosition() {
            ...
            if (offset < 0) {
                ...
                Log.w(TAG, "Update cursor controller position called with no cursor");
                hide();
                return;
            }
